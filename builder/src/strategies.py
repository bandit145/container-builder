...
class Tag(Strategy):
    ...
    def execute(self, cont, **kwargs):
        ...
        for vsn_tag in tag_diff:
            if vsn_tag >= semver.VersionInfo.parse(config['strategy']['args']['version']):
                self.repo.set_branch(f"{config['strategy']['args']['tag_prefix']}{vsn_tag}")
                if list(tag_diff).index(vsn_tag) == 0:
                    build.run(cont, config, tag=f"{config['repo']}:{vsn_tag}", build_repo=self.repo.path, latest=True, build_args={'repo_path': self.repo.path})
                else:
                    build.run(cont, config, tag=f"{config['repo']}:{vsn_tag}", build_repo=self.repo.path, latest=False, build_args={'repo_path': self.repo.path})
